...
class WordTrigger(Trigger):
    ...
    def cleanString(self, story):
        for pun in string.punctuation:
            story = story.replace(pun, " ")
        return story
    
    def isWordIn(self, story):
        story = self.cleanString(story)
        return self.word in story.lower().split()
      # returns True if the word is in story and False if it is not
...
def filterStories(stories, triggerlist):
    ...
    """
    # TODO: Problem 10
    #takes in 2 lists and produces a new list
    filtered_stories = []
    for story in stories:
        for trigger in triggerlist:
            if trigger.evaluate(story):
                if story in filtered_stories:
                    pass          
                else:
