...
def applyShift(text, shift):
    ...
    for char in text:
        if char in string.ascii_lowercase or char in string.ascii_uppercase:
            encrypted_text += buildCoder(shift)[char]
        else:
            encrypted_text += char
    return encrypted_text
...
def findBestShift(wordList, text):
    ...
    """
    ### TODO
    text_words = text.split(' ') # splitting text into words
    max_valid = 0
    best_shift = 0
    num_valid = 0
    for shift in range(26):        
        for word in text_words:
            hidden_text = applyShift(word, shift)
            if isWord(wordList, hidden_text):
                num_valid += 1
...
def decryptStory():
    ...
    wordList = loadWords()
    shift = findBestShift(wordList, Encrypted_story)
    return applyShift(Encrypted_story, shift)
#
